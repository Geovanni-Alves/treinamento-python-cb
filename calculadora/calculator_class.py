...
class Calculator:
    ...
    def _fix_text(self, text):
        # Subistitui tudo que nao for 0123456789./*-+^ para nada
        text = re.sub(r"[^\d\.\/\*\-\+\^\(\)e]", r"", text, 0)
        # Subistitui operadores (sinais) repetidos para apenas 1 sinal
        text = re.sub(r"([\.\+\/\-\*\^])\1+", r"\1", text, 0)
        # Subistitui () ou *() para nada
        text = re.sub(r"\*?\(\)", "", text)
        return text
    ...
    def calculate(self, event=None):
        ...
        try:
            ...
            self.label.config(text="Não consegui realizar a conta, Sorry!")
